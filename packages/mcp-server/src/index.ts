...
Names the file using the standard convention: [Code] - [Name] - [Type] - [Status].`,
  {
    ...
    concept_code: z
      .string()
      .describe("Concept code prefix (e.g. 'K143', 'G109', 'R52')"),
    concept_name: z
      .string()
    ...
    template_file_id: z
      .string()
      .optional()
      .describe("Optional: ID of an existing concept brief to copy as template"),
  },
  async ({
    client_name,
    concept_code,
    ...
    try {
      let file;
      if (template_file_id) {
        file = await drive.copyFile(template_file_id, fileName, briefs_folder_id);
      } else {
...
The sheet can then be populated with script sections, timing, and dialogue.`,
  {
    client_name: z.string().describe("Client brand name"),
    concept_code: z.string().describe("Concept code (e.g. 'K143')"),
    concept_name: z.string().describe("Concept name"),
    folder_id: z.string().describe("Folder ID to create the script in"),
    template_file_id: z
      .string()
      .optional()
      .describe("Optional: ID of an existing script sheet to copy as template"),
    script_content: z
      .string()
      .optional()
      .describe(
  ...
  async ({
    ...
    try {
      let file;
      if (template_file_id) {
        file = await drive.copyFile(template_file_id, fileName, folder_id);
      } else if (script_content) {
        file = await drive.createGoogleDoc(fileName, folder_id, script_content);
      } else {
...
Returns a structured breakdown: metadata, frame-by-frame scene analysis, transcript, and narrative structure.`,
  {
    ...
    frame_detail: z
      .enum(["low", "high"])
      .default("low")
      .describe("Vision API detail level: 'low' (~$0.002/video) or 'high' (~$0.05/video)"),
...
Saves intermediate results for crash recovery at /tmp/video-analysis/.`,
  {
    ...
    brand_context: z
      .string()
      .optional()
      .describe("Brand/product context for creative analysis"),
    resume_from: z
      .string()
      .optional()
      .describe("Path to intermediate results JSON to resume a previous batch"),
    frame_detail: z
      .enum(["low", "high"])
      .default("low")
...
Reads from a saved batch analysis file and writes the formatted output.`,
  {
    analysis_json_path: z
      .string()
      .describe("Path to a saved batch analysis JSON file (from analyze_video_batch)"),
    output_path: z
      .string()
      .describe("Where to save the export"),
    format: z
      .enum(["json", "csv"])
      .default("json")
...
Returns clips organized by scene type (hooks, CTAs, testimonials, product shots, b-roll).`,
  {
    ...
    scene_types: z
      .array(
        z.enum(["hook", "body", "cta", "b-roll", "transition", "testimonial", "product_shot"])
      )
      .optional()
      .describe("Filter to specific scene types (default: all)"),
    min_score: z.number().min(0).max(100).default(30).describe("Minimum quality score (0-100)"),
    top_n_per_type: z.number().min(1).max(50).default(10).describe("Return top N selects per scene type"),
    min_duration: z.number().default(1.0).describe("Minimum clip duration in seconds"),
    base_path: z.string().optional().describe("Base path to prepend to relative video file paths"),
  },
  async ({ analysis_json_path, scene_types, min_score, top_n_per_type, min_duration, base_path }) => {
    try {
      const fs = await import("node:fs");
...
Creates a bin structure ready for Premiere Pro import. Run on existing batch analysis data.`,
  {
    analysis_json_path: z.string().describe("Path to batch analysis JSON file"),
    min_score: z.number().default(30).describe("Minimum score for selects"),
    base_path: z.string().optional().describe("Base path for resolving relative video paths"),
  },
  async ({ analysis_json_path, min_score, base_path }) => {
    try {
      ...
      const service = new PremiereXmlGeneratorService();
      const selects = service.pullSelects(analyses, { minScore: min_score, basePath: base_path });
...
Places clips on a timeline: hook → problem → solution → testimonial → CTA.`,
  ...
  async ({ analysis_json_path, template_name, fps, min_score, base_path }) => {
    try {
      ...
      const selects = service.pullSelects(analyses, { minScore: min_score, basePath: base_path });
      const bins = service.buildBins(selects);
      const template = service.getTemplates().find((t) => t.name === template_name)!;
      ...
      for (let i = 0; i < roughCut.clips.length; i++) {
        ...
        const dur = Math.round((c.timelineOutSeconds - c.timelineInSeconds) * 10) / 10;
...
Combines pull_selects + build_bins + assemble_rough_cut + XMEML generation in one step.`,
  {
    analysis_json_path: z.string().describe("Path to batch analysis JSON file"),
    output_path: z.string().describe("Where to save the .xml file"),
    project_name: z.string().default("AI Rough Cut").describe("Project name shown in Premiere Pro"),
    template_name: z
      .enum([
        "Standard Performance Ad (30s)",
        "Quick Hook Ad (15s)",
    ...
    resolution_height: z.number().default(1920).describe("Output height (1920 for 9:16 vertical)"),
    min_score: z.number().default(30).describe("Minimum quality score"),
    base_path: z.string().optional().describe("Base path to prepend to relative video paths for file:/// URLs"),
  },
  async ({
    analysis_json_path, output_path, project_name, template_name,
    fps, resolution_width, resolution_height, min_score, base_path,
  }) => {
    try {
      const service = new PremiereXmlGeneratorService();
...
function getMimeLabel(mimeType: string): string {
  ...
  return labels[mimeType] || mimeType.split("/").pop() || mimeType;
}
